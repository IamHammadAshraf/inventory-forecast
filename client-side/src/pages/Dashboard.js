...
const Dashboard = () => {
  // User data and role (can be fetched from API)
  const [user, setUser] = useState({
    name: "Hammad Ashraf",
    email: "Hammad@example.com",
    role: "admin", // Can be "customer", "vendor", or "admin"
    status: "Active",
    lastLogin: "2023-05-15 14:30",
    profilePic: "https://media.licdn.com/dms/image/v2/D4D03AQEehiUUXnEKQA/profile-displayphoto-shrink_200_200/B4DZSn59xeHkAc-/0/1737983779446?e=2147483647&v=beta&t=nH-NQ90J7bwU2p7JURCjGnWyGbrmcgH4mGNaf3_HSCk"
  });
  ...
  const products = [
    { id: 1, name: 'Wireless Headphones', category: 'Electronics', stock: 12, price: 99.99 },
    { id: 2, name: 'Smart Watch', category: 'Electronics', stock: 8, price: 199.99 },
    { id: 3, name: 'Bluetooth Speaker', category: 'Electronics', stock: 15, price: 79.99 },
    { id: 4, name: 'USB-C Cable', category: 'Accessories', stock: 42, price: 12.99 }
  ];

  const users = [
    { id: 1, name: 'John Doe', email: 'john@example.com', role: 'vendor', status: 'Active' },
    { id: 2, name: 'Alice Smith', email: 'alice@example.com', role: 'customer', status: 'Active' },
    { id: 3, name: 'Bob Johnson', email: 'bob@example.com', role: 'admin', status: 'Active' },
    { id: 4, name: 'Charlie Brown', email: 'charlie@example.com', role: 'customer', status: 'Inactive' }
  ];
  ...
  const getStats = () => {
    if (user.role === 'customer') {
      ...
      return [
        ...
        { title: 'Inventory Items', value: 42, icon: <FiPackage size={24} />, trend: 'neutral' }
      ];
    } else { // admin
      return [
    ...
    <div className={`min-h-screen ${darkMode ? 'dark bg-gray-900' : 'bg-softCream'}`}>
      ...
      <div className={`fixed inset-y-0 left-0 w-64 p-4 ${darkMode ? 'dark:bg-almostBlack' : 'bg-deepNavy'} text-white`}>
        ...
        <div className="flex items-center mb-8">
          <img src={user.profilePic} alt="Profile" className="w-12 h-12 rounded-full mr-3" />
          <div>
